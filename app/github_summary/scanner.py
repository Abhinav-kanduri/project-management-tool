...
class RepositoryScanner:
    ...
    def scan(self, root: Path) -> RepositorySnapshot:
        ...
        for path in root.rglob("*"):
            ...
            if any(part.lower() in IGNORED_DIRECTORIES for part in relative.parts):
                skipped += 1
                continue
            if is_sensitive_path(relative_text) or not self._is_supported(path):
                skipped += 1
                continue
            try:
    ...
    def _is_supported(path: Path) -> bool:
        return (
            path.suffix.lower() in EXTENSION_LANGUAGE
            or path.name.lower() in TEXT_FILE_NAMES | CONFIG_NAMES
        )
    ...
    def _importance(relative_path: str) -> int:
        ...
        if name.startswith("readme"):
            score += 120
        if name in CONFIG_NAMES or name in ENTRYPOINT_NAMES:
            score += 100
    ...
    def _extract_outline(content: str, language: str) -> list[str]:
        ...
        patterns = [
            ...
            r"^\s*(?:CREATE|ALTER)\s+(?:TABLE|VIEW|FUNCTION|PROCEDURE|INDEX)\s+"
